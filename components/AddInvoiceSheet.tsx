...
export default function AddInvoiceSheet({ open, today, invoices, onClose, onAdd }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    else if (invoices.some((i) => i.invoiceNumber.toLowerCase() === invoiceNumber.trim().toLowerCase()))
  ...
  return (
    <Sheet open={open} onClose={close} title="Add invoice" subtitle="We'll track it against the MSMED Act payment deadline.">
      ...
      <form onSubmit={submit} className="space-y-4" noValidate>
        ...
        >
          {filledFrom ? "Looks good, save invoice" : "Add invoice"}
        </button>
      </form>
    </Sheet>
  );
